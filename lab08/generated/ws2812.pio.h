...
#define bootpin_wrap_target 1
#define bootpin_wrap 3

static const uint16_t bootpin_program_instructions[] = {
...
#if !PICO_NO_HARDWARE
static const struct pio_program bootpin_program = {
    .instructions = bootpin_program_instructions,
    .length = 4,
    .origin = -1,
};

static inline pio_sm_config bootpin_program_get_default_config(uint offset) {
    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_wrap(&c, offset + bootpin_wrap_target, offset + bootpin_wrap);
    return c;
}
